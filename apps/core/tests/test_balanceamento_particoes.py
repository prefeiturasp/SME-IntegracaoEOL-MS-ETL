...
from apps.core.libs.partitioning import (
    dividir_range,
    dividir_range_balanceado,
)


class DividirRangeTest(SimpleTestCase):
    """Testes para dividir_range."""

    def test_divide_em_n_partes(self) -> None:
        partes = dividir_range(1, 100, 4)
    ...
    def test_ultima_particao_vai_ate_id_max(self) -> None:
        partes = dividir_range(1, 8871260, 4)
...
class DividirRangeBalanceadoTest(SimpleTestCase):
    """Testes para dividir_range_balanceado."""
    ...
    def test_retorna_n_particoes(self) -> None:
        ...
        partes = dividir_range_balanceado(hist, 0, 399999, 4, 100000)
        self.assertEqual(len(partes), 4)
    ...
    def test_distribuicao_uniforme_equivale_divisao_linear(self) -> None:
        ...
        partes_bal = dividir_range_balanceado(
            hist, 0, 799999, 4, bucket_size
        )
        partes_lin = dividir_range(0, 799999, 4)
    ...
    def test_dados_assimetricos_equilibram_carga(self) -> None:
        ...
        partes = dividir_range_balanceado(
            hist, 0, 799999, 4, bucket_size
        )
        self.assertEqual(len(partes), 4)
    ...
    def test_histograma_vazio_usa_divisao_linear(self) -> None:
        partes = dividir_range_balanceado([], 1, 100, 4)
        self.assertEqual(partes, dividir_range(1, 100, 4))

    def test_total_zero_usa_divisao_linear(self) -> None:
        hist = [(0, 0), (1, 0)]
        partes = dividir_range_balanceado(hist, 0, 199999, 4, 100000)
        self.assertEqual(partes, dividir_range(0, 199999, 4))

    def test_n_igual_a_1_retorna_range_completo(self) -> None:
        hist = [(0, 100), (1, 200)]
        partes = dividir_range_balanceado(hist, 0, 199999, 1, 100000)
    ...
    def test_dados_concentrados_em_poucos_buckets_usa_fallback(
        ...
        self.assertEqual(partes, dividir_range(0, 399999, 4))

    def test_carga_total_aproximadamente_igual_por_particao(self) -> None:
        """Verifica que a carga por partição está dentro de 50% do alvo."""
        bucket_size = 100_000
        hist = [
            (0, 5), (1, 5), (2, 5), (3, 5),
            (4, 80), (5, 80), (6, 80), (7, 80),
