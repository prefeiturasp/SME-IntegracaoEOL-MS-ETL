...
capacidadeVaga AS (
    SELECT
        ...
        grade.cd_tipo_turno
    FROM turma_escola t
    INNER JOIN serie_turma_escola ON serie_turma_escola.cd_turma_escola = t.cd_turma_escola
...
funcionarios AS (
    ...
    INNER JOIN lotacao_servidor AS lotacao_servidor ON cargoServidor.cd_cargo_base_servidor = lotacao_servidor.cd_cargo_base_servidor
    INNER JOIN v_cadastro_unidade_educacao dre ON lotacao_servidor.cd_unidade_educacao = dre.cd_unidade_educacao
...
SELECT
    ...
    (SELECT dc_dispositivo FROM dispositivo WHERE tp_dispositivo_comunicacao = 9 AND sequencia = 1 AND cd_unidade_educacao = vuedg.cd_unidade_educacao) AS email,
    (SELECT CONCAT('(', cd_ddd, ') ', dc_dispositivo) FROM dispositivo WHERE tp_dispositivo_comunicacao = 1 AND sequencia = 1 AND cd_unidade_educacao = vuedg.cd_unidade_educacao) AS telefone_1,
    (SELECT CONCAT('(', cd_ddd, ') ', dc_dispositivo) FROM dispositivo WHERE tp_dispositivo_comunicacao = 1 AND sequencia = 2 AND cd_unidade_educacao = vuedg.cd_unidade_educacao) AS telefone_2,
    escola.an_construcao AS ano_construcao,
    vuedg.dc_tipo_forma_ocupacao_predio AS propriedade,
    CASE WHEN vuedg.tp_escola IN (11, 12) THEN CAST(1 AS BIT) ELSE CAST(0 AS BIT) END AS organizacao_parceira,
    ISNULL((SELECT quantidadeVagaTurno FROM capacidadeVaga WHERE cd_escola = vuedg.cd_unidade_educacao AND cd_tipo_turno = 1), 0) AS vagas_matutino,
    ISNULL((SELECT SUM(quantidadeVagaTurno) FROM capacidadeVaga WHERE cd_escola = vuedg.cd_unidade_educacao AND cd_tipo_turno IN (3, 4)), 0) AS vagas_vespertino,
    ISNULL((SELECT quantidadeVagaTurno FROM capacidadeVaga WHERE cd_escola = vuedg.cd_unidade_educacao AND cd_tipo_turno = 5), 0) AS vagas_noturno,
    ISNULL((SELECT quantidadeVagaTurno FROM capacidadeVaga WHERE cd_escola = vuedg.cd_unidade_educacao AND cd_tipo_turno = 2), 0) AS vagas_intermediario,
    ISNULL((SELECT quantidadeVagaTurno FROM capacidadeVaga WHERE cd_escola = vuedg.cd_unidade_educacao AND cd_tipo_turno = 6), 0) AS vagas_integral,
    ISNULL((SELECT SUM(quantidadeVagaTurno) FROM capacidadeVaga WHERE cd_escola = vuedg.cd_unidade_educacao), 0) AS vagas_total,
    ISNULL((SELECT quantidade FROM funcionarios WHERE CodigoUnidade = vuedg.cd_unidade_educacao), 0) AS quantidade_funcionarios,
...
class EtlInstitucionalService:
    ...
    def popular_dre(self) -> int:
        ...
        for obj in objs:
            ...
            try:
                ...
                key_cache_dre = f"etl_institucional:dre:{codigo_dre}:codigos_ues_integracao"

                ue_rows = self.eol.executar_query(SQL_OBTER_CODIGOS_UES_POR_DRE, parametros=[codigo_dre])
                ...
                if codigo_ues:
                    ...
                    if novos_mapeamentos:
                        self.cache.set_hash(key_cache_dre, mapping=novos_mapeamentos, expire_seconds=14400)
            except Exception as erro:
                logger.warning(
                    "[ETL] Falha ao popular cache de codigos de integração das UEs para DRE %s: %s", 
                    codigo_dre,
                    str(erro)
                )
                sso_offline = True
        ...
        return _upsert_incremental(DRE, "dre", objs, update_fields)
    ...
    def popular_tipos_escola(self) -> int:
        ...
        return _upsert_incremental(TipoEscola, "tipo_escola", objs, update_fields)
    ...
    def popular_subprefeituras(self) -> int:
        ...
        return _upsert_incremental(SubPrefeitura, "sub_prefeitura", objs, update_fields)

    def popular_unidades_educacionais(self) -> int:
        """Fase 4: Extrai e persiste UnidadeEducacional com enriquecimento segmentado por DRE."""
