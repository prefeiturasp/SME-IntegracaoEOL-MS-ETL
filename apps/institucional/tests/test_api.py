"""Testes da API de institucional."""
...
class HealthInstitucionalViewTestCase(TestCase):
    """Testes para endpoints do HealthInstitucionalView."""
    ...
    def test_health_sem_variavel(self) -> None:
        """Sem URL_BANCO_INSTITUCIONAL deve retornar unhealthy."""
        response = self.client.get("/api/v1/institucional/health/")

        self.assertEqual(response.status_code, 503)
        self.assertEqual(response.json()["status"], "unhealthy")

    @patch.dict(os.environ, {"URL_BANCO_INSTITUCIONAL": "postgres://teste"})
