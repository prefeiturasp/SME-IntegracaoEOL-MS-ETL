...
class EtlInstitucionalServiceTestCase(TestCase):
    ...
    def test_popular_dre_insere_novos(self) -> None:
        ...
        self.assertEqual(resultado, 1)
        self.assertTrue(
            DRE.objects.using("institucional_db")
            .filter(codigo_dre="108900")
            .exists()
        )
    ...
    def test_popular_unidades_educacionais_incremental_por_hash(self) -> None:
        ...
        row_original = (
            "000001", "UE 1", "UE 1", "EMEF", "R", "Rua 1", "1", "B", "000",
            "SP", "D", "e@e.com", "123", None, 2020, "P", False, 10, 10,
            0, 0, 0, 20, 5, "A", "108900", 1, 1,
        )
        row_alterada = (
            "000001", "UE 1 ALTERADA", "UE 1", "EMEF", "R", "Rua 1", "1", "B",
            "000", "SP", "D", "e@e.com", "123", None, 2020, "P", False, 10,
            10, 0, 0, 0, 20, 5, "A", "108900", 1, 1,
        )
    ...
    def test_executar_fluxo_completo(self) -> None:
        ...
        self.mock_eol.executar_query.side_effect = [
            ...
            [("001", "UE", "UE", "T", "L", "LOG", "1", "B", "00", "SP", "D", "e", "1", None, 2020, "P", False, 0, 0, 0, 0, 0, 0, 0, "A", "108900", 1, 10)] # 4: SQL_UNIDADE_EDUCACIONAL
        ]
        self.mock_sso.obter_codigos_integracao_ues.return_value = [("001", "UE", "INT-001")]
    ...
    def test_popular_dre_trata_erro_sso(self) -> None:
        """Garante que falha no SSO não interrompe o ETL das DREs (cobertura try/except)."""
        self.mock_eol.executar_query.side_effect = [
            [("108900", "DRE BT", "BT", 1, "Reg")],
            [("001",)]
        ]
        # Simula erro no repositório legado
        self.mock_sso.obter_codigos_integracao_ues.side_effect = Exception("Banco offline")
        
        resultado = self.service.popular_dre()
        
        # Deve persistir a DRE normalmente apesar do erro no enriquecimento
        self.assertEqual(resultado, 1)
        self.assertTrue(DRE.objects.using("institucional_db").filter(codigo_dre="108900").exists())

    def test_popular_dre_interrompe_tentativas_se_sso_offline(self) -> None:
        """Garante que falha no SSO no primeiro item marca offline e interrompe tentativas nos demais."""
        self.mock_eol.executar_query.side_effect = [
            [
                ("108900", "DRE BT", "BT", 1, "Reg"),
        ...
        # O popular_dre deve tentar processar a primeira DRE, falhar no SSO, marcar como offline e BREAK no segundo item.
        resultado = self.service.popular_dre()
        
        # Ambos DREs devem ser persistidos (o erro de SSO não para o insert das DREs, só o enriquecimento do cache)
